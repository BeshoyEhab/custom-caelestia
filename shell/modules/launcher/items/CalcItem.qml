...
Item {
    ...
    readonly property string math: {
        const text = list.search.text.trim();
        const prefix = `${GlobalConfig.launcher.actionPrefix}calc `;
        if (text.startsWith(prefix))
            return text.slice(prefix.length);
        // Auto-detected math expression (no prefix)
        return text;
    }
    ...
    RowLayout {
        ...
        StyledRect {
            ...
            StateLayer {
                ...
                onClicked: {
                    Quickshell.execDetached(["app2unit", "--", ...GlobalConfig.general.apps.terminal, "qalc", "-i", root.math]);
                    root.list.visibilities.launcher = false;
                }
